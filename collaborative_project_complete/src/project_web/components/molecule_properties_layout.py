...
def heavy_atoms_component(raw_data):
    graph = ""
    data_stats = heavy_atoms.get_data(raw_data)
    if not data_stats:
        return ("", {})
    graph = heavy_atoms.draw_component(data_stats["data"])
        
    return (html.Div([html.H5("Number of heavy atoms per molecule"),
                     html.Div(graph)],
                    id="heavy-atoms-div",
                    className='six columns'),
            data_stats)
