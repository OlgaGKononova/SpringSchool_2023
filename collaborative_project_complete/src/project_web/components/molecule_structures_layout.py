...
def select_mol_structures(raw_data, prop_ranges, num_mols=30):
    ...
    #s_min, s_max = similarity_range
    #smiles_list = [d["molecule_structures"]["canonical_smiles"] for d in raw_data if s_min <= d["similarity"] <= s_max]
...
# def sample_mol_structures(mol_data, num_mols=20):
#     smiles_list = similar_structures.get_structures(mol_data, num_mols)
#     encoded_sample_img = draw_molecules_grid_from_smiles(smiles_list)
    
#     return [html.Label("Most similar structures"),
#             html.Img(src="data:image/png;base64, " + encoded_sample_img,
#                      id="sim-mols-img", 
#                      className="image")]  
