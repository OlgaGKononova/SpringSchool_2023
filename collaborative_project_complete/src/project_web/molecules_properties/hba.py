...
def get_data(raw_data: list) -> dict:
    ...
    """
    hba_values = [d["molecule_properties"]["hba"] for d in raw_data if d["molecule_properties"]["hba"]]
    return dict(component="H-bond acceptors",
                data=hba_values,
...
def draw_component(data_array: list) -> dcc.Graph:
    ...
    plot = [go.Histogram(x=data_array,
                         # x=[k for k, v in pd_data.items()],
                         # y=[v for k, v in pd_data.items()],
                         marker={"color": "#00b0f0",
                                 "line": {"width": 3,
                                          "color": "#028DBF"}},
