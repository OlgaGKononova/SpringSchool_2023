...
def get_data(raw_data: list) -> dict:
    ...
    """
    ro5_violations_values = [d["molecule_properties"]["num_ro5_violations"] for d in raw_data if d["molecule_properties"]["num_ro5_violations"]]
    return dict(component="Number of 'rule of five' violations",
                data=ro5_violations_values,
                mean=np.mean(ro5_violations_values),
                std=np.std(ro5_violations_values),
                max_value=np.max(ro5_violations_values),
                min_value=np.min(ro5_violations_values),
                )
...
def draw_component(data_array: list) -> dcc.Graph:
    ...
    """
    #pd_data = pd.Series(data_array).value_counts()
    
    plot = [go.Histogram(x=data_array,
                        #  labels = [k for k, v in pd_data.items()],
                        #  values = [v for k, v in pd_data.items()],
                        #  hole=.3
                         marker={"color": "#002060",
                                 "line": {"width": 3,
                                          "color": "#011745"}},
