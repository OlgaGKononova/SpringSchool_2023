...
def get_data(raw_data: list) -> dict:
    ...
    """
    aromatic_rings_values = [d["molecule_properties"]["aromatic_rings"] for d in raw_data if d["molecule_properties"]["aromatic_rings"]]
    return dict(component="Number of aromatic rings",
                data=aromatic_rings_values,
                mean=np.mean(aromatic_rings_values),
                std=np.std(aromatic_rings_values),
                max_value=np.max(aromatic_rings_values),
                min_value=np.min(aromatic_rings_values),
                )
...
def draw_component(data_array: list) -> dcc.Graph:
    ...
    plot = [go.Histogram(x=data_array,
                         #y=[k for k, v in pd_data.items()],
                         #x=[v for k, v in pd_data.items()],
                         # orientation='h',
                         marker={"color": "#92d050",
                                 "line": {"width": 3,
                                          "color": "#73A340"}},
