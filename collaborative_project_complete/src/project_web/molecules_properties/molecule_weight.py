...
def get_data(raw_data: list) -> dict:
    ...
    """
    mol_weight_values = [d["molecule_properties"]["full_mwt"] for d in raw_data if d["molecule_properties"]["full_mwt"]]
    return dict(component="Molecule weight",
                data=mol_weight_values,
                mean=np.mean(mol_weight_values),
                std=np.std(mol_weight_values),
                max_value=np.max(mol_weight_values),
                min_value=np.min(mol_weight_values),
                )
